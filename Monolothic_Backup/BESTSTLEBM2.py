...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  4 17:38:25 2025

@author: apm


this is after BESTSTLEBM1 after exams 

named BESTSTLEBM2


??? this is question
&&& this is somehting that must be checked later
$$$ this is imrpovment



after final button --> we can disable all things
...
0-->geometry from right --> nothing i see

1---> multi object -->  i do something but we must test for previewe 
2----> start heat , jump safe and other based on energy 
3---> obp to obp.gz ( in obp saving and also jason) --> i compress all , also + .gz for jason build info

in multiple -> if only 1 --> it say only 1 and n_ --> 1
if detect one but is multiple --> it must decrease the overlapp and try again



future-------
all thingsa must be oin Try --> each tab 

also we must talk about possibility of changing the colour of themese

also when you pass to next section the previous must be disabled all the buttons


after building it must have preview button for that 
...
from PM_EBM_CORE2 import Core_Mesh_Processor,Core_Heat_Processor,Core_OBF_Generator
...
# Custom TabBar class for coloring
...
class ProjectSetupDialog(QDialog):
    def __init__(self):
        '''
        First Windows contains information related to project
        ...
        self.license.setText("All rights reseved to APM.")
...
'''
&&&
also chekc the p and P rject after that
...
class STLManipulatorTabs(QMainWindow):
    ...
    def create_box(self):
        '''
        For building the Box if we need other AM process
    ...
    def create_ebm_base(self):
        '''
        function for building the basement for objects in EBM
    ...
    def setup_geometry_tab(self):
        '''
        Create tabs for geometry (stl) and working with things
        it can also has import that
        if we have other format conevrt to stl
...
""")
    ...
    def apply_multiply(self):
        ...
        if self.click_on_apply_multiple==True:
            ...
            if self.it_is_final_multiples==True:
    ...
    def slicing(self):
        ...
        it has only self.test_slice_planes
        self.slice_n_layers
        self.slice_layer_height
        
        
        slef --> successive and ... (they called in analyze)
    ...
    def slice_analyzing(self):
        '''
        here we just get the update 
        
        self.number_of_successive_layer
        
        self.number_of_failed_slice
        
        self.number_of_failed_geom
        
        self.failed_slice_id
...
""")
   ...
   #animation
    ...
    def j_line_algorithem(self):
        #it must ge and tehn call the spatter
        pass
    ...
    def old_Final_preheating(self):
        #here we must add somethign for spatters...
    ...
    def Final_preheating(self):
        # Here we must add something for spatters...
    ...
    def setup_hatching_tab(self):
        '''
        first it has different tab from raster and point and ....
        
        each tab has soem parameters and buttons (hatch, analyze and...)
        
        Here thats we are going to the anything that we want
    ...
    def create_raster_tab(self):
        '''
        Raster tab --> it is linear
        if has raster_hatching , analyhzing , final
    ...
    def raster_hatching(self):
        ...
        self.test_hatched_layers
        
        self.test_whole_points
        self.test_whole_lines
        
        self.number_of_failed_hatch_line
        
        
        #---finaly with final it get us
        self.test_hatched_layers
        self.whole_points
        self.whole_lines
...
class AdvancedWindow_layers(QWidget):
    ...
    def display_layer_parameters(self, index):
        """Update UI to show parameters for the selected layer."""
        if index < 0:
...
class AdvancedWindow_layers1(QWidget):
    ...
    def create_scroll_section(self, title, count, create_function):
        """Creates a scrollable section for layer parameters."""
    ...
    def create_layer_parameters(self, form):
        """Creates layer-specific parameters."""
        heat_algo = QComboBox()
        heat_algo.addItems(["Heat1", "Heat2"])
        form.addRow("Heat Algorithm:", heat_algo)
