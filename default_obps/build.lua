    ...
    while not layerDone do
        ...
        if layer.jumpSafe ~= nil then
            for _, obp in ipairs(layer.jumpSafe) do
                ...
                system.print("Total Jump Safe reps: " .. math.max(0, obp.repetitions + currentJumpSafeReps) .. "")
                table.insert(jumpSafePatterns, { file = obp.file, repetitions = math.max(0, currentJumpSafeReps + obp.repetitions) })
            end
        end
        ...
        if layer.heatBalance ~= nil then
            for _, obp in ipairs(layer.heatBalance) do
                ...
                system.print("Total Heat Balance reps: " .. math.max(0, obp.repetitions + heatBalanceRepetitions) .. "")
